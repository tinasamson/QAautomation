describe('Shopping Cart', () => {
    ...
    cy.get('@allProducts').then((allProducts) => {
      ...
      cy.randomNum(numProducts).then((randomNum) => {
        cy.wrap(allProducts)
          ...
          .should('exist')
          .invoke('text')
          .as('productPrice', { type: 'static' });
        cy.wrap(allProducts)
          .find('.productinfo p')
          .eq(randomNum)
          .should('exist')
          .invoke('text')
          .as('productName', { type: 'static' });
        cy.wrap(allProducts)
          .find('.productinfo a')
          .eq(randomNum)
      ...
      cy.get('@productCartOverview').then((productCartOverview) => {
        cy.wrap(productCartOverview)
          .find('.cart_description h4')
          .invoke('text')
          .as('productCartName', { type: 'static' });
        cy.wrap(productCartOverview)
          .find('.cart_price p')
          .invoke('text')
          .as('productCartPrice', { type: 'static' });
      });
  ...
  it('test case 7: Add multiple items of same product from product detail page to cart', () => {
    ...
    cy.get('@productDetails').then((productDetails) => {
      cy.wrap(productDetails)
        .find('h2')
        .invoke('text')
        .as('productDetailName', { type: 'static' });
      cy.wrap(productDetails)
        .find('p')
        .first()
        .invoke('text')
        .as('productDetailCategory', { type: 'static' });
      cy.wrap(productDetails)
        .find('span span')
        .invoke('text')
        .as('productDetailPrice', { type: 'static' });
      cy.wrap(productDetails).find('input#quantity').clear();
      cy.wrap(productDetails)
        .find('input#quantity')
        .type(quantityProd.toString());
    ...
    cy.get('#cart_info_table tbody tr')
      .eq(0)
      .then((productCartOverview) => {
        cy.wrap(productCartOverview)
          .find('.cart_total_price')
          .invoke('text')
          .as('productCartTotalPrice', { type: 'static' });
        ...
        cy.get('@productDetailCategory').then((productCategory) => {
          cy.wrap(productCartOverview)
            .find('.cart_description p')
            .invoke('text')
            .then((overviewCategory) => {
  ...
  it('test case 8: Add multiple products from product list page to cart', () => {
    cy.gotoAEUrl('/products');
    cy.get('.product-image-wrapper').as('allProducts');
    ...
    cy.get('@allProducts').then((allProducts) => {
      ...
      cy.twoRandomNum(numProducts).then((randomNumbers) => {
        ...
        for (let i = 1; i < 3; i++) {
          ...
          cy.wrap(allProducts)
            ...
            .should('exist')
            .invoke('text')
            .as('product' + i.toString() + 'Name', { type: 'static' });
          cy.wrap(allProducts)
            .find('.productinfo h2')
            .eq(randomNum)
            .should('exist')
            .invoke('text')
            .as('product' + i.toString() + 'Price', { type: 'static' });
          cy.wrap(allProducts)
            .find('.productinfo a')
            .eq(randomNum)
            .should('have.text', 'Add to cart')
      ...
      for (let i = 1; i < 3; i++) {
        cy.get('#cart_info_table tbody tr')
          ...
          .then((productCartOverview) => {
            cy.wrap(productCartOverview)
              .find('.cart_description h4')
              .invoke('text')
              .as('productCart' + i.toString() + 'Name', { type: 'static' });
            cy.wrap(productCartOverview)
              .find('.cart_price p')
              .invoke('text')
              .as('productCart' + i.toString() + 'Price', { type: 'static' });
            cy.wrap(productCartOverview)
              .find('.cart_total p')
              .invoke('text')
              .as('productCart' + i.toString() + 'PriceTotal', {
                type: 'static',
              });
          });
        // validate product information in cart overview
        cy.get('@productCart' + i.toString() + 'Name').then(
          (productCartName) => {
        ...
        cy.get('@productCart' + i.toString() + 'Price').then(
          (productCartPrice) => {
        ...
        cy.get('@productCart' + i.toString() + 'PriceTotal').then(
          (productCartPrice) => {
            cy.get('@product' + i.toString() + 'Price').then((productPrice) => {
              expect(productCartPrice).to.eq(productPrice);
            });
