...
class ProductPage(Utils):
    ...
    def validateAllProducts(self):
        ...
        totalProducts = len(
            self.driver.find_elements(By.CSS_SELECTOR, ".product-image-wrapper")
        )
        assert totalProducts > 0
        return totalProducts

    def getProductNamePrice(self, randomIndex):
        productPrice = self.driver.find_elements(
            By.CSS_SELECTOR, ".productinfo h2"
        )[randomIndex].text
    ...
    def validateProductInfo(self, randomIndex):
        assert self.driver.find_elements(By.CSS_SELECTOR, ".productinfo a")[
            randomIndex
        ].text, "Add to cart"
        viewProductBtn = self.driver.find_elements(By.CSS_SELECTOR, ".choose")[
            randomIndex
        ]
        assert viewProductBtn.text, "View Product"

    def viewProduct(self, randomIndex):
        self.driver.find_elements(By.CSS_SELECTOR, ".choose")[
            randomIndex
        ].click()
    ...
    def getCategory(self):
        self.driver.find_element(
            By.CSS_SELECTOR, ".category-products"
        ).is_displayed()
        categories = self.driver.find_elements(
            By.CSS_SELECTOR, '.category-products [data-toggle="collapse"]'
        )
    ...
    def getCategoryFilters(self, idCategoryName):
        filters = self.driver.find_elements(
            By.CSS_SELECTOR, "#" + idCategoryName + " .panel-body ul li a"
        )
    ...
    def getBrand(self):
        self.driver.find_element(
            By.CSS_SELECTOR, ".brands_products"
        ).is_displayed()
        assert (
            self.driver.find_element(
                By.CSS_SELECTOR, ".brands_products h2"
            ).text
            == "BRANDS"
        )
        brandFilters = self.driver.find_elements(
            By.CSS_SELECTOR, ".brands_products li a"
        )
    ...
    def validateBrandFilter(self, brandName, numBrandProd):
        assert (
            brandName in self.driver.find_element(By.CLASS_NAME, "title").text
        )
        assert (
            len(
    ...
    def searchProduct(self, searchText):
        self.driver.find_element(
            By.CSS_SELECTOR, "input#search_product"
        ).send_keys(searchText)
        self.driver.find_element(
            By.CSS_SELECTOR, "button#submit_search"
        ).click()

    def validateSearch(self, searchText):
        assert (
            self.driver.find_element(By.CLASS_NAME, "title").text
            == "SEARCHED PRODUCTS"
        )
        productsTitle = self.driver.find_elements(
            By.CSS_SELECTOR, ".product-image-wrapper .productinfo p"
        )
